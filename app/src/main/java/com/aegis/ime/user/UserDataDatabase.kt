...
internal class UserDataDatabase private constructor(
    ...
    fun checkpointLastGood() {
        try {
            ...
            File(root, LAST_GOOD_DIGEST_NAME).writeText(sha256(lastGoodFile) + "\n")
            lastFailure = null
        } catch (e: Exception) {
    ...
    private fun writeRecoveryStatus(report: UserDataRecoveryReport) {
        ...
        val tmp = File(root, "$STATUS_NAME.tmp")
        tmp.writeText(text)
        atomicReplace(tmp, statusFile)
    }
    ...
    companion object {
        ...
        fun open(root: File): UserDataDatabase {
            ...
            if (main.isFile && !isValidDatabase(main)) {
                val restored = lastGood.isFile && isValidDatabase(lastGood)
                if (restored) {
                    val tmp = File(root, "$DATABASE_NAME.restore")
                    copyFile(lastGood, tmp)
                    atomicReplace(tmp, main)
                    deleteCompanions(main)
                    report = UserDataRecoveryReport(UserDataRecoveryKind.LAST_GOOD, "restored verified last-good snapshot")
                } else {
                    main.delete()
                    deleteCompanions(main)

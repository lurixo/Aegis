...
/**
 * Clipboard-history + canned-phrases panel (issue #7). Two tabs over a
 * scrollable list of entries; tapping one commits it. Data is pulled live from the providers so it
 * reflects the latest captured clips each time the panel opens.
 */
class ClipboardView(context: Context) : LinearLayout(context) {
    ...
    /** Rebuild rows for the current tab — call when the panel becomes visible. */
    fun refresh() {

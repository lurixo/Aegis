...
/**
 * Symbol-side pinyin associations (U23 data expansion): every nameable [com.aegis.ime.layout.SymbolCatalog]
 * symbol, keyed by the toneless full pinyin of its natural Chinese name(s). Pure data — only
 * [InputAssociations] consumes it; the symbols panel/catalog rendering is untouched.
 *
 * Coverage contract (locked by InputAssociationsDataTest): every catalog symbol is either reachable through
 * a [rows] key (or one of the pre-existing legacy keys) or listed in [exemptions] with its reason — nothing
 * may be skipped silently. Glyph lists are most-common-first and at most [InputAssociations.MAX_PER_QUERY]
 * long, so every listed glyph actually surfaces for its own key.
 *
 * [rows] is a FUNCTION, not a stored val: the Row objects are consumed once by the [InputAssociations]
 * merge (and by the data tests) and then garbage-collected, so the table is not retained twice.
 */
internal object SymbolAssociations {

    /** One nameable symbol family: [name] = hanzi name(s) (review aid; "" = same family as the row above),
     *  [keys] = space-separated toneless full-pinyin keys, [glyphs] = targets, most common first. */
    ...
    val exemptions: List<Exemption> = listOf(
        ...
        Exemption("点式/括号式序号变体,与圈N同义无区分性自然名", "⒈", "⒉", "⒊", "⒋", "⒌", "⒍", "⒎", "⒏", "⒐", "⒑", "⑴", "⑵", "⑶", "⑷", "⑸", "⑹", "⑺", "⑻", "⑼", "⑽"),

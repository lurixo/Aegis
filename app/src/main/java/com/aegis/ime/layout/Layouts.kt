...
object Layouts {
    ...
    private const val NINE_RIGHT_U = 0.7f // Chinese IME behavior note.
    ...
    private fun qwerty(lang: Lang): KeyboardLayout {
        ...
        val bottom = listOf(
            Key("符号", action = SHOW_SYMBOLS, weight = 1.5f), // Chinese IME behavior note.
    ...
    fun nine(lang: Lang, left: List<Key>, composing: Boolean = false): KeyboardLayout {
        ...
        cells.add(PlacedKey(Key("符号", action = SHOW_SYMBOLS), xL, 0.75f, wL, 0.25f)) // Chinese IME behavior note.

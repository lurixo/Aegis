...
/**
 * The user-dictionary page: live word/pinyin search over a lazily-composed list (thousands of self-created
 * words stay smooth), manual add/delete, and import/export of the SAME userdb.txt the IME learns into.
 *
 * List model: [UserDictEdit.list] is read once on entry and after each mutation; typing in the search box
 * only re-filters that in-memory list ([UserDictSearch.filter]) — the userdb file is never re-parsed per
 * keystroke. Every edit dispatches through [UserDictEdit], so while the IME service is running the change
 * lands in its LIVE model (visible on the next keystroke) and is persisted in the same call.
 *
 * The management tools (import/export, manual add) are list headers that hide while a query is active, so
 * search results sit directly under the pinned search field.
 */
@Composable
internal fun UserDictPage(onBack: () -> Unit) {
    ...
    // The custom (recall) words: loaded once and after every edit/import (never per search keystroke).
    // While the IME service is up this list is its live model, so it also reflects fresh auto-learning.
    ...
    fun applyImport(uri: Uri, merge: Boolean) {
        val ok = runCatching {
            ...
            // ④ The apply step validates the import and never wipes the live dict on failure. Through the
            // live host it merges into the in-use model, so unsaved learning survives an import too.
    ...
    // Insets OUTSIDE the lazy list (same principle as settingsScrollInsets): the viewport shrinks to the
    // keyboard top, so the pinned search field and the list rows are never hidden behind the IME.
    Column(
        modifier = Modifier
            .fillMaxSize()
                ...
                item(key = "tools") {
                    Card(modifier = Modifier.fillMaxWidth()) {
                        Column(
                            ...
                            Button(
                                onClick = {
                                    // Persist unsaved live learning first, so the exported file is current.

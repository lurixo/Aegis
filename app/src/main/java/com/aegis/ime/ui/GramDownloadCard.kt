...
/**
 * E1 — 增强模型下载管理. Three states: 未下载[下载] → 下载中[进度] → 已下载[下载置灰 + 更新 + 删除].
 * The single `downloaded/` path + atomic rename mean re-download/更新 overwrite rather than duplicate;
 * 删除 is thorough (.gram + leftover .part + stored validator). 更新 is offered only when a HEAD shows
 * the remote release differs from the locally recorded validator (or the remote can't be compared).
 */
@Composable
internal fun GramDownloadCard() {
    ...
    // When a model is present, ask the server (HEAD) whether its release differs from what we hold,
    // so 更新 lights up only for a genuinely newer file (fall back to allowing a
    // forced re-download if the remote can't be compared).

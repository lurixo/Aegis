...
/** The user-dictionary page's search semantics — pure in-memory filtering, incl. the spec'd boundaries. */
class UserDictSearchTest {
    ...
    // ---- boundary: pinyin prefix ----

    @Test fun pinyin_prefix_matches_all_readings_it_prefixes() {
        assertEquals(listOf("你好", "你好呀"), words("nih"))
        assertEquals(listOf("你好", "你好呀"), words("nihao")) // full reading is a prefix of itself
    ...
    @Test fun cjk_query_never_matches_readings() {
        // A CJK query is not pinyin: it must not fall back to letter matching.
        assertEquals(emptyList<String>(), words("好a好")) // mixed junk: word-substring only, no reading match
    }

    // ---- scale: thousands of entries stay a flat in-memory filter ----
    ...
    @Test fun ten_thousand_entries_filter_correctly() {
        ...
        val hits = UserDictSearch.filter(big, "aa") // letters "aa" anywhere in the reading

...
/**
 * debug.47: every settings-page user-dict operation dispatches through [UserDictHot] — LIVE host first
 * (IME running → immediate effect), file fallback second (no IME → the existing next-onStartInput pickup).
 * This pins the dispatch itself; the live host's semantics are covered in LiveUserDictHostTest.
 */
class UserDictEditDispatchTest {
    ...
    @Test fun without_a_host_operations_fall_back_to_the_file_and_round_trip() {
        ...
        UserDictEdit.flushBeforeExport() // no host → must be a safe no-op
    }
}

...
/**
 * §1 (jank root cause, "SymbolsView/EmojiView tab 切换同款治理"). The old showCategory did
 * grid.removeAllViews + a fresh tile (FrameLayout/TextView) + RippleDrawable per glyph on EVERY tab switch.
 * These prove the tab grids recycle their tiles (allocation tops out at the peak category, a re-sweep allocates
 * nothing), rebind content correctly, and — the risky reuse path — a pooled symbol tile that rendered an
 * auto-sized multi-char token before renders a fixed wide/normal glyph after (auto-size toggled off), and its
 * corner badge appears/hides per tab. The clipboard case proves the 剪贴板↔常用语 tab switch fades exactly once.
 */
...
class PanelRecyclingTest {
    ...
    @Test fun emoji_grid_allocates_at_peak_not_sum_across_a_full_sweep() {
        ...
        for (i in 0..EmojiCatalog.categories.size) v.openCategoryForTest(i) // 0=recent(empty) + every catalog tab
        val afterSweep1 = v.emojiCellsAllocatedForTest()
        for (i in 0..EmojiCatalog.categories.size) v.openCategoryForTest(i) // second full sweep
    ...
    @Test fun emoji_grid_content_and_tap_are_correct_after_recycling() {
        ...
        v.openCategoryForTest(2) // warm the pool on a different category first
    ...
    @Test fun reused_symbol_tile_toggles_autosize_off_for_a_fixed_wide_glyph() {
        var recent = listOf("arcsin") // multi-char → auto-size UNIFORM
        ...
        // Re-render the SAME slot with a single wide glyph — the pooled view must switch auto-size OFF and take
        // the fixed, wide-scaled size (else the old auto-size config would linger and mis-size the glyph).
        recent = listOf("℃")
        v.openCategoryForTest(0)
    ...
    @Test fun reused_symbol_tile_hides_the_badge_when_moving_off_the_recent_tab() {
        ...
        v.openCategoryForTest(0) // 常用 tab: "$" carries a 货 badge
        ...
        v.openCategoryForTest(idx(catId)) // a catalogue tab: tiles carry no badge
    ...
    @Test fun symbol_tap_reports_the_origin_after_recycling() {
        ...
        // A catalogue category with a plain single-glyph symbol (skip 'net', whose url tokens ride the chip bar).
        val catNo = SymbolCatalog.categories.indexOfFirst { it.id != "net" && it.symbols.any { s -> s.length == 1 } }
        v.openCategoryForTest(idx("net")) // warm on a different tab first
        v.openCategoryForTest(catNo + 1) // rail index = catalogue index + 1 (recent tab is 0)

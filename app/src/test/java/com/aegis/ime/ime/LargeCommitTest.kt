...
/** E5: the chunked-commit logic must reproduce the EXACT text (no truncation) in binder-safe pieces. */
class LargeCommitTest {

    @Test fun million_char_text_chunks_reassemble_exactly_and_stay_under_the_limit() {
        val text = buildString { repeat(1_000_000) { append('a' + (it % 26)) } } // 1,000,000 chars

...
/**
 * ② Multi-touch per-pointer tracking on the self-drawn [KeyboardView]: rolling (a second finger down before
 * the first lifts) must commit BOTH keys, in order, on BOTH keyboards, for EVERY release order — the old code
 * ignored ACTION_POINTER_DOWN/UP and dropped the rolled key. Mechanically enumerates key pairs and lift orders.
 */
...
class KeyboardMultiTouchTest {
    ...
    // --- multi-pointer MotionEvent construction (Robolectric on the JVM) ---
    ...
    /**
     * A two-finger roll: finger 1 presses ([ax],[ay]); finger 2 presses ([bx],[by]) BEFORE finger 1 lifts;
     * then the two lift in [aFirst] (true = finger 1 lifts first) order. Returns the emitted key outputs.
     */
    private fun KeyboardView.roll(ax: Float, ay: Float, bx: Float, by: Float, aFirst: Boolean): List<String> {
        ...
        dispatch(pointerDown(1), 10, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // f2 lands
        if (aFirst) {
            dispatch(pointerUp(0), 20, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // f1 up first
            dispatch(MotionEvent.ACTION_UP, 30, intArrayOf(2), floatArrayOf(bx), floatArrayOf(by))    // f2 last
        } else {
            dispatch(pointerUp(1), 20, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // f2 up first
            dispatch(MotionEvent.ACTION_UP, 30, intArrayOf(1), floatArrayOf(ax), floatArrayOf(ay))    // f1 last
        }
        return out
    }
    ...
    @Test fun rolling_two_digits_on_the_9key_commits_both_in_order_either_lift_order() {
        for (aFirst in listOf(true, false)) {
            val v = nineView()
            val (ax, ay) = v.centerOfLabelForTest("ABC")!! // emits "2"
            val (bx, by) = v.centerOfLabelForTest("DEF")!! // emits "3"
    ...
    @Test fun mechanical_enumeration_all_pairs_of_a_key_block_roll_cleanly() {
        // Enumerate every ordered pair of the 9-key digit block (9 keys → 72 ordered pairs) × both lift orders.
        val v0 = nineView()
    ...
    @Test fun three_finger_roll_commits_all_three_in_press_order() {
        ...
        val (ax, ay) = v.centerOfLabelForTest("ABC")!! // 2
        val (bx, by) = v.centerOfLabelForTest("DEF")!! // 3
        val (cx, cy) = v.centerOfLabelForTest("GHI")!! // 4
    ...
    @Test fun a_second_finger_retap_while_first_rests_does_not_double_emit() {
        // Regression (audit MAJOR): finger 1 holds X; finger 2 taps Y (POINTER_DOWN commits X, POINTER_UP
        // commits Y, activePointerId → INVALID because finger 1 still rests); finger 2 taps Y AGAIN. The second
        // POINTER_DOWN must NOT re-finish the stale (nulled) down key — it would spuriously re-emit Y.
        val v = nineView()
        val out = ArrayList<String>()
        v.onKey = { out.add(it.output) }
        val (ax, ay) = v.centerOfLabelForTest("ABC")!! // X → "2"
        val (bx, by) = v.centerOfLabelForTest("DEF")!! // Y → "3"
        v.dispatch(MotionEvent.ACTION_DOWN, 0, intArrayOf(1), floatArrayOf(ax), floatArrayOf(ay)) // finger 1 holds X
        v.dispatch(pointerDown(1), 10, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // finger 2 → Y (commits X)
        v.dispatch(pointerUp(1), 20, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // finger 2 up (commits Y; finger 1 still down)
        v.dispatch(pointerDown(1), 30, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // finger 2 re-taps Y
        v.dispatch(pointerUp(1), 40, intArrayOf(1, 2), floatArrayOf(ax, bx), floatArrayOf(ay, by)) // finger 2 up (commits Y once)
        v.dispatch(MotionEvent.ACTION_UP, 50, intArrayOf(1), floatArrayOf(ax), floatArrayOf(ay)) // finger 1 finally lifts
    ...
    @Test fun sequential_non_overlapping_taps_still_commit_both() {
        // Regression: the single-pointer tap path must be unchanged (no overlap → plain ACTION_UP each time).
        val v = nineView()
        val out = ArrayList<String>()
        v.onKey = { out.add(it.output) }

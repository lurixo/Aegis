...
/**
 * ③ mis-touch tightening boundary enumeration: the tightened snap cap (drops taps > half a key away) and the boundary-based
 * slide retarget (only a slide that LEAVES the pressed key retargets). Robolectric drives real MotionEvents.
 */
...
class KeyboardMistouchTest {
    ...
    // --- snap cap: gap taps still land; far-outside taps are dropped ---
    ...
    @Test fun a_tap_in_a_narrow_inter_key_gap_still_snaps_to_a_key() {
        ...
        val gapX = (ax + bx) / 2f // the 6dp gutter between two 9-key digit cells
    ...
    @Test fun a_tap_just_below_the_bottom_row_within_half_a_key_still_lands() {
        ...
        val bottom = sy + (rowHeight * 0.30f) // ~0.3 key height below the space key's centre → still under it or within cap
    ...
    @Test fun a_tap_well_below_the_keyboard_is_dropped() {
        ...
        val farBelow = v.height + rowHeight // a full key height below the whole keyboard → beyond the half-key cap
    ...
    // --- boundary-based retarget: micro-move stays, crossing the boundary retargets ---
    ...
    @Test fun a_micro_move_within_the_pressed_key_still_commits_that_key() {
        val v = nineView()
        val (ax, ay) = v.centerOfLabelForTest("ABC")!! // output "2"
        // Move a few px but stay well inside ABC's rectangle → must still commit ABC.
    ...
    @Test fun a_slide_onto_the_neighbour_retargets_slide_to_correct() {
        val v = nineView()
        val (ax, ay) = v.centerOfLabelForTest("ABC")!!
        val (bx, by) = v.centerOfLabelForTest("DEF")!! // output "3"
        // Deliberately slide from ABC's centre onto DEF's centre (past the boundary) → retargets to DEF.
    ...
    @Test fun a_slide_that_stops_at_the_shared_edge_does_not_flip() {
        val v = nineView()
        val (ax, ay) = v.centerOfLabelForTest("ABC")!!
        // Move right to just inside ABC's right edge (within the hysteresis band) → must NOT flip to DEF.
        val p = v.centerOfLabelForTest("ABC")!!
        // half a cell width minus a couple dp keeps us inside ABC (cells are ~1 unit ≈ 68px wide here).

...
/**
 * Level B: what the USER sees through [KeyboardController] when typing en. The decoder-level
 * guarantees (EnVisibilityFixTest) must survive the real controller path on both layouts:
 * 26-key letters "en", 9-key digits 36 unlocked, 9-key with the reading en locked — 嗯 present in the
 * strip each time, ranked after the top native 恩, with ng/64 behaving exactly as before.
 * The full-pack config re-runs everything when `AEGIS_FULLDICT_DIR` (+`AEGIS_GRAM`) is set.
 */
...
class EnVisibilityUiTest {
    ...
    private fun assertStripOffers嗯(dir: File, gram: OctagramReader?, cfg: String) {
        ...
        run { // 9-key digits, unlocked — the fault A path
            val c = controller(dir, gram)
        ...
        run { // ng regression: 9-key 64 keeps 嗯 up front
            val c = controller(dir, gram)
            c.onKey(Key("", action = KeyAction.SWITCH_NINE))
            type(c, "64")

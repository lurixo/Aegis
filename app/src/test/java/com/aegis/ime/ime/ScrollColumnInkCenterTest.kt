...
/**
 * ④ Per-symbol INK-CENTRE assertion for the self-drawn scroll column, on BOTH boards (9-key pinyin/punctuation
 * column and numpad operator column — the SAME drawScrollColumn code path). Robolectric NATIVE graphics
 * rasterises the real onDraw; for each visible cell the test measures the drawn glyph's ink bounding box and
 * asserts its centre sits on the cell centre. Uses discriminating Latin glyphs (".", "g", "-", "|") whose ink
 * is deliberately asymmetric in the em box, so a regression back to font-line-box centring (which offsets a
 * bottom-heavy "." / a descender "g") is caught, plus the real punctuation/operator marks for coverage.
 */
...
class ScrollColumnInkCenterTest {
    ...
    private val u = 1f / 4.7f // NINE_LEFT_U(1.0) | 1 | 1 | 1 | 0.7 = 4.7 units
    ...
    /** Ink bounding box (dark label pixels) of the axis-aligned rect [x0,x1)×[y0,y1); null if no ink found. */
    private fun inkBox(bmp: Bitmap, x0: Int, y0: Int, x1: Int, y1: Int): IntArray? {
        ...
        for (y in y0 until y1) for (x in x0 until x1) {
            ...
            if ((r + g + b) / 3 < 110) { // dark label ink over the light track
    ...
    /** Measure each visible scroll-column cell's ink centre vs its cell centre. [hPx] is the rendered height. */
    private fun measureColumn(v: KeyboardView, labels: List<String>, hPx: Int, regionH: Float): List<Cell> {
        ...
        val scrollbar = (6f * density).toInt() // keep the right scrollbar thumb out of the ink scan
    ...
    @Test fun discriminating_latin_glyphs_are_ink_centred_on_both_axes() {
        // ".", "g", "-", "|": ink deliberately NOT in the em-box centre. Font-line-box centring would offset the
        // bottom-heavy "." and the descender "g"; ink centring must place every glyph's ink box on the cell centre.
        val labels = listOf(".", "g", "-", "|")
    ...
    @Test fun pinyin_punctuation_column_marks_are_ink_centred() {
        // The real resting 9-key left column (，。？！ …). Full-width CJK ink sits low-left in the em box; ink
        // centring keeps each mark inside a tight band around the cell centre.
    ...
    @Test fun numpad_operator_column_marks_are_ink_centred() {
        // The numpad operator column shares drawScrollColumn — the same fix must cover it. +, −, ×, ÷ visible.
        ...
        // numpad opCol spans the FULL height (regionH = 1f), 4 visible.

...
/**
 * Lock boundary — an explicit lock is a HARD boundary: dictionary whole words whose own syllabification cannot
 * respect the forced cuts are filtered from the locked decode (锁 fang+an 不出 反感), words whose
 * syllabification aligns are kept (方案), and words whose syllabification is unrecoverable offline
 * (猩猩: 猩 has no standalone entry) are kept — crossing cannot be proven. Free typing (no cuts) is
...
class LockBoundaryTest {
    ...
    // ---------- the specified behaviour: positive/negative cases ----------

    @Test fun lockFangAn_filtersFanGanWord_keepsFangAnWord() {
        val d = decoder()
        val locked = words(d.decodeCoveredAtomic("fangan", 30, setOf(4))) // lock fang|an
    ...
    @Test fun lockFanGan_filtersFangAnWord_keepsFanGanWord() {
        val d = decoder()
        val locked = words(d.decodeCoveredAtomic("fangan", 30, setOf(3))) // lock fan|gan
    ...
    @Test fun lockMinGan_filtersMingAnWords() {
        val d = decoder()
        val locked = words(d.decodeCoveredAtomic("mingan", 30, setOf(3))) // lock min|gan
        assertTrue("锁 min+gan 不得出现 明暗 (ming+an): $locked", "明暗" !in locked)
        assertTrue("锁 min+gan 不得出现 命案 (ming+an): $locked", "命案" !in locked)
        // and the dual: locking ming|an keeps them but filters min+gan-aligned words
        val dual = words(d.decodeCoveredAtomic("mingan", 30, setOf(4))) // lock ming|an
        assertTrue("锁 ming+an 保留 明暗/命案: $dual", "明暗" in dual || "命案" in dual)
        assertTrue("锁 ming+an 不得出现 敏感 (min+gan): $dual", "敏感" !in dual)
    }

    // ---------- free typing must be UNCHANGED ----------
    ...
    // ---------- recall must not collapse: aligned words survive their own lock ----------
    ...
    @Test fun alignedCommonWords_surviveTheirLock() {
        ...
        val checks = listOf(
            ...
            Triple("chaici", setOf(4), "拆次"), // the KeyboardLossMatrixTest sentinel
        )
    ...
    @Test fun unprovableWords_areKept() {
        val d = decoder()
        // 猩猩: 猩 has no standalone single entry in the seed — crossing unprovable → kept under its lock
        val got = words(d.decodeCoveredAtomic("xingxing", 30, setOf(4)))
        assertTrue("反查不出切分的词(猩猩)疑罪从无保留: ${got.take(8)}", "猩猩" in got)
    }

    // ---------- full-config targeted check (gated) ----------

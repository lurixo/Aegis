...
/**
 * Ranking-quality harness: top-1 / top-5 sentence accuracy on a held-out set of
 * (pinyin, expected-hanzi) pairs, comparing unigram-only against the char-bigram LM
 * across a λ sweep. Prints a table and writes build/eval_report.txt. The eval set is
 * generated + verified out of band into src/test/resources/eval_set.tsv.
 */
class RankingEvalTest {

    // Defaults are the shipped demo assets; override via env to measure against the full dict.
    ...
    fun reportRankingQuality() {
        ...
        // The bigram should not regress top-1 at its best λ (it may strongly improve it).

...
/**
 * Usage-boost rise-curve MEASUREMENT harness (report-only; the decoder is not modified and no
 * ranking behaviour is gated here). Answers, with production-config numbers, the user question
 * "how many times must I pick a word before it moves up / becomes the default?".
 *
 * Everything is measured through the REAL candidate-strip surface the keyboards render —
 * `PinyinDecoder.decodeCovered(reading, 30)` (30 = `DictEngine.MAX_CANDIDATES`), production
 * wiring: full downloadable dict pack + bundled char-bigram LM + jianpin initials dict + the
 * downloadable octagram model; fuzzy OFF (the shipped default). The 9-key decoder mirrors
 * `DictEngine`: digit dict + `aliasDict` = the letter dict, candidates filtered to non-ASCII.
 * No score arithmetic is re-derived anywhere: every reported count c* is bracketed by real
 * decodes and boundary-verified (predicate true at c*, false at c*-1), so each number is a
 * reproducible pair of observations, not a model prediction.
 *
 * Sweeps (all mechanically enumerated from the runtime syllable set, no curated word lists):
 *  S1 same-reading single-char homophones — every runtime syllable; every ADJACENT pair of the
 *     visible strip (rank i+1 overtaking rank i, incl. rank2-over-rank1) plus, per anchor
 *     (strip top = high-frequency start, strip midpoint = mid-frequency start), EVERY single in
 *     the frequency band around gap targets 2x/5x/10x/100x (tolerance +-5%), full in-band
 *     enumeration.
 *  S2 same-key multi-char words — every ordered 2-syllable key (415^2) carrying >=2 multi-char
 *     entries: the (top, second) pair plus the same anchor/band enumeration over the key's words.
 *  S3 sentence default vs word — every 2-syllable key whose natural #0 is a COMPOSED sentence
 *     (not a whole-key dict entry) but which carries a whole-key word: picks of the top word
 *     until it seizes the commit default; plus a strided 3-syllable extension.
 *  S4 self-created-word recheck — the five fixed witness words of the user-dictionary rise-curve
 *     audit, re-measured on the CURRENT tree in that audit's exact configuration (seed dict + LM
 *     + jianpin, no octagram) and again on the full production configuration; plus the
 *     rise-to-#2/#3 counts.
 *
 * Gating/config (environment): AEGIS_BOOST_REPORT=1 enables (skipped otherwise — a plain suite
 * run never pays for this); AEGIS_FULLDICT_DIR = directory holding the full-pack
 * aegis_dict.bin/aegis_t9.bin/aegis_jianpin.bin; AEGIS_GRAM = octagram model path;
 * AEGIS_AUDIT_DIR = output directory for TSVs + summaries; AEGIS_BOOST_THREADS (default 4);
 * AEGIS_BOOST_SMOKE=1 shrinks every universe for a wiring check.
 */
class BoostCurveReportTest {

    // ---------- config + shared assets (companion: loaded once per JVM across test methods) ----------
    private companion object {
        ...
        const val STRIP_LIMIT = 30      // DictEngine.MAX_CANDIDATES — what a user can actually see/pick
        const val COUNT_CAP = 65536     // counts above this are reported as censored (">cap")
        const val T9_STRIDE = 37        // 1-in-37 of the cases (by stable case-identity hash, so the
                                        // selection is identical across runs/threads) re-measured on 9-key

        /** Deterministic 9-key spot-check membership for a case identity (reading+challenger). */
        fun t9Probe(caseId: String): Boolean = caseId.hashCode().mod(T9_STRIDE) == 0
        val RATIO_TARGETS = listOf(2.0, 5.0, 10.0, 100.0)
        const val RATIO_TOL = 0.05      // +-5% band around each target gap
    ...
    // ---------- per-thread decode context (dict/LM/octagram shared & read-only; model + decoders private) ----------
    private class Ctx(dict: BinaryDict, t9Dict: BinaryDict, jianpin: BinaryDict?, oct: OctagramReader?) {
        val um = UserModel()
        // production 26-key wiring (AegisInputMethodService.buildEngine -> DictEngine): fuzzy OFF default
        val letter = PinyinDecoder(dict, lm, userModel = um, initialsDict = jianpin, octagram = oct)
        // production 9-key wiring: aliasDict = the letter dict
    ...
    private fun stripOf(ctx: Ctx, t9: Boolean, reading: String): List<String> =
        if (!t9) {
            ...
            // DictEngine.candidatesCovered(t9=true) parity: digits in, stray ASCII lattice strings filtered
            ctx.t9.decodeCovered(T9Pinyin.toT9(reading), STRIP_LIMIT).map { it.word }
                .filterNot { w -> w.all { it.code < 128 } }
        }

    /**
     * The measured predicate at usage count [c]: record [b] c times ([selfReading] != null routes through the
     * self-created-word path `recordWord`, mirroring word assembly; null mirrors a plain candidate commit
     * `record`), decode the strip, restore the model, and test — [a] == null: b IS the commit default (#0);
     * else: b sits strictly ahead of a (a fallen off the visible strip while b shows also counts as ahead).
     */
    ...
    /** b's strip index at usage count [c] (-1 = not visible). */
    ...
    /**
     * Minimal count satisfying [pred] (monotone in c by construction — the boost is the only moving part and
     * is strictly increasing), found by exponential bracket + binary search on REAL decodes, then
     * boundary-verified: pred(c*) && !pred(c*-1) re-observed explicitly. Returns c* or -1 (censored: still
     * false at [COUNT_CAP]). The verified flag is false only if the monotonicity assumption itself broke.
     */
    ...
    // ---------- streamed TSV + compact aggregation (values only; rows never held in heap) ----------
    private class Sink(file: File, header: String) {
        private val w = file.bufferedWriter()
        private val agg = ConcurrentHashMap<String, MutableList<Int>>() // binKey -> c* values
    ...
    private fun <T> runParallel(tasks: List<Callable<T>>) {
        ...
        try {
            ...
            for (f in futures) f.get() // surface the first worker exception, if any
        } finally {
            pool.shutdownNow()
        }
    }
    ...
    // =====================================================================================
    // S1 — same-reading single-char homophones, per-syllable full enumeration
    // =====================================================================================
    @Test fun s1_singles_sameReading_riseCurve() {
        ...
        runParallel(syls.map { s ->
            Callable {
                val ctx = tl.get()
                // own-key singles + (mirroring the decoder's input aliases) the alias target's singles
                ...
                // adjacent visible pairs (rank i+1 over rank i) — the strip order users actually see
                for (i in 0 until vis.size - 1) {
                    measure("adjacent", if (i == 0) "top" else "deeper", i, vis[i], vis[i + 1])
                }
                // anchor x gap-band pairs: full enumeration of every single inside each ratio band,
                // from the whole homophone list (visible or not); alias-discounted words excluded from
                // bands (their strip frequency is not their raw dict frequency)
    ...
    // =====================================================================================
    // S2 — same-key multi-char words over all 2-syllable keys
    // =====================================================================================
    ...
    // =====================================================================================
    // S3 — composed-sentence default vs the key's top whole-key word
    // =====================================================================================
    @Test fun s3_sentenceDefault_vsWord_riseCurve() {
        ...
        fun probe(ctx: Ctx, key: String, nSyl: Int) {
            ...
            if (words.any { it.word == nat }) return // dict-word default: S2 territory
            val b = multi[0]
        ...
        // strided 3-syllable extension (illustrative, mechanically generated)
        val n = syls.size
    ...
    // =====================================================================================
    // S4 — self-created-word recheck (prior audit witnesses, current tree) + rise to #2/#3
    // =====================================================================================
    @Test fun s4_selfCreatedWord_recheck() {
        ...
        val configs = buildList {
            // the prior user-dictionary audit's exact configuration: seed pack + LM + jianpin, no octagram
            add(Config("seed-noOctagram", seedDict, seedT9, seedJianpin, null))
            // today's full production configuration
        ...
        for (cfg in configs) {
            ...
            for ((reading, word) in cases) {
                ...
                // a word the dict already carries never takes the self-created storage path
                // (DictEngine.learnWord skips it) — its usage is then plain dict-word learning
                ...
                if (!inDict) {
                    // the recall floor guarantees a self-created word is always visible for its reading;
                    // a dict-carried word may legitimately sit below the 30-candidate fold at low counts

...
class DictEngineFuzzyT9Test {
    ...
    @Test fun the_digit_rule_table_mirrors_every_letter_rule() {
        assertEquals(Fuzzy.RULES.map { it.key }, T9Pinyin.FUZZY_RULES.map { it.key })
        for ((letters, digits) in Fuzzy.RULES.zip(T9Pinyin.FUZZY_RULES)) {
            assertEquals(T9Pinyin.toT9(letters.long), digits.long)
            assertEquals(T9Pinyin.toT9(letters.short), digits.short)
            assertEquals(letters.initial, digits.initial)
            assertNotEquals(
                "a rule whose two sides collide on the keypad would expand to nothing",
                digits.long,
                digits.short,
            )
        }

...
class NineNumpadAlignmentTest {
    ...
    private val gapPx = 6f * dm.density // KeyboardView gap
    ...
    fun longestSyllableShowsFullyInTheLeftReadoutColumn() {
        ...
        // The drawn region (mirrors KeyboardView.relayout) and the available width drawScrollColumn fits text in.
        val colWpx = sc.w * wPx - 2 * gapPx
        val avail = colWpx - 12f * dm.density // == drawScrollColumn's `scrollRegion.width() - 12*density`
        val base = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 17f, dm) // scrollLabelPaint sp(17)
        fun widthAt(size: Float) = Paint().apply { textSize = size }.measureText("zhuang")
        val w0 = widthAt(base)
        // Replicate the KeyboardView font-fit: shrink ONLY if over-wide, clamped to 11sp.
        ...
        // The widened column (Layouts.NINE_LEFT_U = 1.0) means the shrink, if any, is small — it stays readable.
    ...
    fun numpadSharesThePinyinNineKeyMetrics() {
        ...
        // item6: the operator scroll column is the SAME width as the pinyin left read-out column.
        ...
        // item6: a main-grid cell is the same size + column as a pinyin letter cell (digit 5 vs letter ABC, both
        // main column 2).
        ...
        // item7: the @ key is gone.
        assertTrue("numpad has no @ key", cells(numpad).none { it.key.label == "@" })

        // item8: ↵ is the tall right-column key spanning rows 2-3 (y=0.5, h=0.5) — identical to the pinyin enter.

...
/**
 * Coverage + variant audit, against the bundled Unicode emoji-test.txt v16.0 (the RGI oracle, in
 * src/test/resources — not shipped). No sampling: every assertion walks the WHOLE catalog / variant set / RGI
 * set. This is independent of the generator — the catalog is static data; this test re-derives the expected
 * set straight from the Unicode file, so a dropped cell, a bad variant string or a shrunk coverage口径 fails
 * the build mechanically.
 *
 * 口径 (coverage criterion): reachable = the catalog's default cells, plus each cell's man/woman gender forms,
 * plus the five uniform skin tones of every reachable form. That covers 3486 of the 3781 fully-qualified RGI
 * emoji; the 295 not covered are exactly the per-person independent skin-tone combinations of the 13 two-person
 * couple / handshake emoji (e.g. 🧑🏻‍🤝‍🧑🏿) — the uniform-tone selector offers a single tone, matching standard
 * keyboards; per-person tone matrices are the deliberate out-of-scope combinatorial tail.
 */
class EmojiVariantsDataTest {
    ...
    // person / hand scalars that make an emoji "multi-person" (couple / handshake) when two are present.
    ...
    /** Reachable = cells ∪ each cell's gender forms ∪ the uniform skin tones of every reachable form. */
    ...
    // ---------------------------------------------------------------- catalog integrity (no invented glyphs)
    ...
    // ---------------------------------------------------------------- skin-tone rule (every output RGI)
    ...
    // ---------------------------------------------------------------- full coverage口径 (mechanical, no sampling)

    @Test fun every_base_emoji_is_reachable_and_coverage_matches_the_stated口径() {
        val reach = reachable()
        // 1) every fully-qualified non-skin-tone (base) emoji is reachable as a cell or a gender variant.
        ...
        // 2) exact coverage of the whole RGI set.
        ...
        // 3) NOTHING without a skin tone is excluded — the only gaps are per-person tone combinations.
    ...
    @Test fun excluded_forms_are_exactly_per_person_multitone_couples() {
        ...
        // strip all tone modifiers → the underlying two-person base

...
val debugVersionName = "0.1.0-debug.80"
val debugVersionCode = 9150

androidComponents {
    onVariants(selector().withBuildType("debug")) { variant ->
        variant.outputs.forEach {
            it.versionName.set(debugVersionName)
            it.versionCode.set(debugVersionCode)
        }
    }
}
...
tasks.withType<Test>().configureEach {
    ...
    inputs.file(
        providers.environmentVariable("AEGIS_COVERAGE_DIGEST_BASELINE")
            .map(::file)
            .orElse(layout.projectDirectory.file("src/test/resources/coverage-identity-68354991.tsv").asFile),
